...
public class WUGraph {
	...
	public int degree(Object vertex) {
		if (adjList.find(vertex) != null) {
			return ((HashTableChained) ((Entry) adjList.find(vertex)).value())
					.size();
		}
		return 0;
	}
	...
	public Neighbors getNeighbors(Object vertex) {
		...
		HashTableChained table = (HashTableChained) ((Entry) adjList
				.find(vertex)).value();
		...
		for (int i = 0; i < edges.length; i++) {
			...
			nb.weightList[i] = (int)((Integer)((Entry) table.find(edges[i])).value());
		}
		return nb;
	}
	...
	public int weight(Object u, Object v) {
        if (isEdge(u, v)) {
            return (int)((Integer) ((Entry) ((HashTableChained) ((Entry) adjList.find(u)).value()).find(v)).value());
        }
        return 0;
    }
}
